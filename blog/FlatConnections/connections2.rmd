...
With this point of view, a general "global" proof of the correspondence can be given by using the geometric statement of the Frobenius theorem. We are going to sketch this proof, but I prefer to study in detail the proof for matrix groups, which is more simple and straightforward, and maybe fits better with the "cocycle approach" that I have been maintaining in my previous posts.
...
for $\alpha: U \rightarrow \mathbb{R}^n$ and some open set $U\subset \mathbb{R}^m$. The set of points $N=\{(t,\alpha(t)) \in \mathbb{R}^{m+n} : t \in U\}$ forms a smooth submanifold of $\mathbb{R}^{m+n}$ with tangent vectors

$$
\left. \frac{\partial}{\partial t^i} \right|_t = \left. \frac{\partial}{\partial t^i} \right|_{(t,\alpha(t))} + \frac{\partial \alpha}{\partial t^i} (t) =\left. \frac{\partial}{\partial t^i} \right|_{(t,\alpha(t))} + F_i(t,\alpha(t)) = X_i.
$$
...
That is, our problems is reduced to that of finding solutions $f$ to the differential equation
...
which the $dx^\nu \wedge dx^\mu$ component of the form $F_A=dA + A \wedge A$. Since by assumption the connection is flat, the integrability condition is met and solutions exist.
